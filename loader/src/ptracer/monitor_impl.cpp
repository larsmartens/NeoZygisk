...
void AppMonitor::run() {
    ...
    // Scan for already-running zygote processes that were forked before the
    // monitor started. This handles Magisk Alpha where post-fs-data.sh runs
    // after zygote has already been spawned by init.
    ptrace_handler_.ScanExistingZygotes();
    event_loop_.Init();
...
/**
 * @brief Scans /proc for already-running zygote processes and initiates injection.
 *
 * On some root implementations (e.g., Magisk Alpha), the monitor's post-fs-data.sh
 * executes AFTER init has already forked the zygote process. Since PTRACE_SEIZE with
 * PTRACE_O_TRACEFORK only captures future fork events, the existing zygote is missed.
 *
 * This method compensates by scanning /proc for processes that:
 *   1. Are direct children of init (PPID == 1)
 *   2. Are running the target program (e.g., /system/bin/app_process64)
 *   3. Are not already being traced (TracerPid == 0)
 *
 * For each found zygote, it triggers the same injection handoff sequence used
 * for newly-forked zygotes: SIGSTOP the process, then fork the injector daemon.
 */
void AppMonitor::SigChldHandler::ScanExistingZygotes() {
    if (monitor_.get_tracing_state() != TRACING) return;

    const auto &target_program = monitor_.get_abi_manager().program_path_;
    LOGI("scanning /proc for existing zygote processes (%s)", target_program.c_str());

    DIR *proc_dir = opendir("/proc");
    if (!proc_dir) {
        PLOGE("opendir /proc for zygote scan");
        return;
    }
    ...
    while ((entry = readdir(proc_dir)) != nullptr) {
        ...
        // Check PPID == 1 (direct child of init) via /proc/[pid]/stat
        char path_buf[64];
        snprintf(path_buf, sizeof(path_buf), "/proc/%ld/stat", pid);
        ...
        if (fgets(line, sizeof(line), f)) {
            ...
            // Find the closing ')' of comm (handles parens/spaces in comm)
            char *cp = strrchr(line, ')');
            if (cp) {
                char state;
                sscanf(cp + 2, "%c %d", &state, &ppid);
            }
        }
        fclose(f);
        ...
        // Check if this process is running the target program
        auto program = get_program(static_cast<int>(pid));
        if (program != target_program) continue;

        // Check TracerPid == 0 (not already being traced)
        ...
        if (tracer_pid != 0) {
            LOGV("skipping process %ld: already traced by %d", pid, tracer_pid);
            continue;
        }

        LOGI("found existing zygote at PID %ld, initiating late injection", pid);

        const char *tracer = monitor_.get_abi_manager().check_and_prepare_injection();
        if (tracer == nullptr) {
            LOGE("failed to prepare injector for existing zygote %ld", pid);
            continue;
        }

        // SIGSTOP the zygote so the injector can PTRACE_SEIZE it in a stopped state
        kill(static_cast<int>(pid), SIGSTOP);

        // Brief delay to ensure SIGSTOP is delivered before the injector attaches
        usleep(100000);  // 100ms

        // Fork and execute the injector daemon (same handoff as handleExecEvent)
        auto p = fork_dont_care();
        if (p == 0) {
            execl(tracer, basename(tracer), "trace", std::to_string(pid).c_str(), "--restart",
                  nullptr);
            PLOGE("execute injector for existing zygote");
            kill(static_cast<int>(pid), SIGKILL);
            _exit(1);
        } else if (p == -1) {
            PLOGE("fork injector for existing zygote");
            // Unfreeze the zygote if we failed to spawn the injector
            kill(static_cast<int>(pid), SIGCONT);
        }
